...
int druck = 0;                                          // Zustandsvariable für Beenden des Spiels
...
{ 
  if(dauer < x.Laenge)                      // Dauer des Spieles wird geprüft
  {
    if(((ID[1]-ID[0]) > 900)&&((ID[1]-ID[0]) < 1100)&&((ID[2]-ID[1]) > 400)&&((ID[2]-ID[1]) < 600)&&((ID[21]-ID[20]) > 400)&&((ID[21]-ID[20]) < 600))       // Validieren des Start- und Stop-Bits
    {
      ...
      if(stat[0]&&stat[1]&&stat[2]&&stat[3]&&stat[4]&&stat[5]&&stat[6]&&stat[7]&&!stat[8]&&x.name9!="")
      {
      ...
      else if(stat[0]&&stat[1]&&stat[2]&&stat[3]&&!stat[4]&&!stat[5]&&!stat[6]&&!stat[7]&&!stat[8]&&x.name5!="")
      {
...
// Magazin-Reload \\
    ...
    if(digitalRead(Menue_Pin) == HIGH)
    {
      druck++;
      delay(10);
    }
    else druck = 0;
    if(druck == 500) digitalWrite(Enable_Pin, LOW);
  }
  else
  {
    display.clear_screen();
