...
void setup()
{
  ...
  display.return_home();
  display.set_ramposition(PCF2119_DISPLAY_START_POSITION);
  display.printf("%-10s Lives: %d", x.name10.c_str(), Leben);
...
{ 
  ...
  {                                               
    if(((ID[1]-ID[0]) > 900)&&((ID[1]-ID[0]) < 1100)&&((ID[2]-ID[1]) > 400)&&((ID[2]-ID[1]) < 600)&&((ID[21]-ID[20]) > 400)&&((ID[21]-ID[20]) < 600))       // Validieren des Start- und Stop-Bits
    {
      ...
      if(stat[0]&&stat[1]&&stat[2]&&stat[3]&&stat[4]&&stat[5]&&stat[6]&&stat[7]&&!stat[8]) Hit(8);
...
void Hit(int value)
{
  ...
  {
    display.printf("Killed by %s", x.name9.c_str());
